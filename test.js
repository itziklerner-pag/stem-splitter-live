...
if (group('live')) {
   ...
   * branch time and 44/51 later, and seven of those seven gaps were DEAD ANCHORS
   * rather than weak assertions. Re-establish these numbers; never quote them:
   *
   *   node qa/mutations-u7-live-recording.mjs              # run them all
   *   node qa/mutations-u7-live-recording.mjs --list       # do the anchors still MATCH?
   *   node qa/mutations-u7-live-recording.mjs --self-check # can this battery say MUTE?
   *   node qa/mutations-u7-live-recording.mjs --coverage   # is any assertion here unclaimed?
   *   node qa/mutations-u7-live-recording.mjs --table      # regenerate the rows below
   *
   * THE ROWS BETWEEN THE TWO MARKERS ARE GENERATED BY `--table`, not typed, so
   * the table and the battery cannot drift apart by hand. `file:line` is where
   * the MUTATION was made, never where the red appeared, and the `->` line names
   * the assertion keys (see `A` in the battery) that went red — EXACTLY those,
   * no more: an unclaimed red means the table is wrong about what a line covers.
   * `x6` marks a key emitted once per (hop, stop-point) pair, i.e. six reds.
   *
   ...
   *                                 -> chunkafter
   *   M6   engine/live.js:216       gap() after finish() appends unseparated audio to a finished recording
   *                                 -> gapafter
   *   M7   engine/live.js:308       finish() AT the commit point publishes a zero-length span instead of refusing
   *                                 -> atcommit
   *   M8   engine/live.js:312       finish() accepts a span longer than one hop plus the crossfade
   *                                 -> beyond
   *   M9   engine/live.js:333       finish() compresses a whole crossfade into a short drain (makeFades(len)) instead of ending mid-fade
   *                                 -> midfade
   *   M10  engine/live.js:357       finish() keeps the tail, so a second join could be published over frames already in the file
   ...
   *
   *   M3 is this phase's worked example of a gate that REWARDS THE DEFECT. Under
   *   the identity model of the 18 residual assertions below, the held tail and
   *   the final pass are the SAME samples, so complementary linear ramps sum to
   *   a no-op: deleting the crossfade entirely leaves every one of them GREEN
   *   with the residual at -inf dB, which reads as an improvement rather than as
   *   a defect. Only the DELTA-step instrument — two passes differing by a known
   *   constant over a silent input — can see it. ANY GATE WHOSE FIXTURE MAKES
   *   TWO INPUTS IDENTICAL IS BLIND TO WHATEVER DISTINGUISHES THEM.
   *
   *   M10 reds NOTHING, and is kept rather than deleted. `haveTail = false`
   *   after `finished` is unreachable while M4's guard stands, so it is defence
   *   in depth and not a covered line. A battery that quietly dropped the row
   *   would be claiming coverage it does not have.
   *
   *   M34 THROWS, and its one red is the group guard below. That is evidence the
   *   GUARD reports — not evidence that anything downstream of the throw is
   *   watching: those assertions are ABSENT, and an absent assertion reads as
   *   green. It is the bound stated three paragraphs up, measured.
   */
   ...
   */
  {
    ...
    /**
     * IT NAMES THE WRITER, and that is not pedantry — it is a coverage loss this
     * assertion had already suffered and nobody would have seen. When ruling 34
     * gave the recording its own writer, `skipOne()` gained a SECOND
     * `noteDrop()` call two lines below the prime's. A bare
     * `skipBody.includes('noteDrop()')` is then satisfied by the RECORDING's
     * call, so deleting `this.cacheWriter.noteDrop()` — the exact defect this
     * assertion exists to catch — left it GREEN. Watched: the mutation went from
     * a red to a pass the moment the second writer landed. This is the silent
     * decay INTEGRATION.md §18 describes in its quietest form, an assertion that
     * keeps passing while its subject is removed, and the fix is to say WHICH
     * writer rather than to check that some writer somewhere was told.
     */
    ok('...and a PASSTHROUGH SPAN IS COUNTED ON THE PRIME\u2019S WRITER, not only aborted — the refusal can name how many  '
      + '[entry point: extension/offscreen/live.js skipOne(), the call site shared/stemcache.js noteDrop() names]',
      skipBody.includes('this.cacheWriter.noteDrop()') && skipBody.includes('this.cacheWriter.abort()')
      && skipBody.indexOf('this.cacheWriter.noteDrop()') < skipBody.indexOf('this.cacheWriter.abort()'),
      !skipBody.includes('this.cacheWriter.noteDrop()')
        ? 'skipOne() aborts the prime without counting the drop, so a refusal cannot say how many chunks were lost'
        : !skipBody.includes('this.cacheWriter.abort()')
          ? 'skipOne() counts the drop but never abandons the prime — a gap gets cached as a track'
          : 'cacheWriter.noteDrop() then cacheWriter.abort(), both, in that order');
  }
